...
function GoogleSignInButton() {
  ...
  const handleClick = () => {
    ...
    window.google.accounts.id.prompt((notification) => {
      // If One Tap is not displayed or skipped, the button still works via prompt
      if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
        console.log('Google One Tap not available, user can try again');
      }
    });
  };
  ...
  return (
    <button
      ...
      className="w-full flex items-center justify-center gap-3 px-6 py-3 rounded-xl bg-white/[0.07] hover:bg-white/[0.12] border border-white/10 hover:border-white/20 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed group"
    >
      {loading ? (
