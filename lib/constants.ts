...
export const ALL_MODELS: ModelDefinition[] = [
  ...
  // Extra open source models to approach 1000+
  ...generateExtraModels(),
];

function generateExtraModels(): ModelDefinition[] {
  const extras: ModelDefinition[] = [];
  const openSourceBases = [
    { provider: 'huggingface', prefix: 'huggingfaceh4', models: ['zephyr-7b-beta', 'zephyr-7b-alpha', 'starchat2-15b-v0.1'] },
    { provider: 'teknium', prefix: 'teknium', models: ['openhermes-2.5-mistral-7b', 'openhermes-2-mistral-7b'] },
    { provider: 'openchat', prefix: 'openchat', models: ['openchat-3.5-0106', 'openchat-3.6-8b'] },
    { provider: 'phind', prefix: 'phind', models: ['phind-codellama-34b'] },
    { provider: 'cognitivecomputations', prefix: 'cognitivecomputations', models: ['dolphin-2.6-mixtral-8x7b', 'dolphin-2.5-mixtral-8x7b'] },
    { provider: 'mistralai', prefix: 'mistralai', models: ['mistral-tiny', 'mistral-embed'] },
    { provider: 'meta-llama', prefix: 'meta-llama', models: ['llama-2-7b-chat'] },
    { provider: 'google', prefix: 'google', models: ['gemma-7b-it', 'gemma-2b-it', 'palm-2-chat-bison', 'palm-2-codechat-bison'] },
    { provider: 'anthropic', prefix: 'anthropic', models: ['claude-2.1', 'claude-2.0', 'claude-instant-1.2'] },
    { provider: 'openai', prefix: 'openai', models: ['gpt-4-0613', 'gpt-4-0314', 'gpt-3.5-turbo-0125', 'gpt-3.5-turbo-1106', 'gpt-3.5-turbo-16k'] },
    { provider: 'ai21', prefix: 'ai21', models: ['jamba-1.5-large', 'jamba-1.5-mini', 'j2-ultra', 'j2-mid'] },
    { provider: 'aleph-alpha', prefix: 'aleph-alpha', models: ['luminous-supreme', 'luminous-extended', 'luminous-base'] },
    { provider: 'amazon', prefix: 'amazon', models: ['titan-text-express-v1', 'titan-text-lite-v1'] },
    { provider: 'fireworks', prefix: 'fireworks', models: ['firellama-13b', 'firefunction-v2'] },
    { provider: 'together', prefix: 'together', models: ['stripedhyena-nous-7b', 'together-llm-chat-7b'] },
    { provider: 'inflection', prefix: 'inflection', models: ['inflection-3-pi', 'inflection-3-productivity'] },
    { provider: 'xai', prefix: 'xai', models: ['grok-2', 'grok-2-mini', 'grok-beta'] },
    { provider: 'writer', prefix: 'writer', models: ['palmyra-x-004', 'palmyra-x-003-instruct'] },
    { provider: 'snowflake', prefix: 'snowflake', models: ['snowflake-arctic-instruct', 'snowflake-arctic-embed-l'] },
    { provider: 'alibaba', prefix: 'alibaba', models: ['qwen-max', 'qwen-plus', 'qwen-turbo'] },
  ];

  for (const base of openSourceBases) {
    for (const model of base.models) {
      extras.push({
        provider: base.provider,
        modelId: `${base.prefix}/${model}`,
        displayName: model.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' '),
        category: model.includes('code') || model.includes('coder') ? 'code' : model.includes('embed') ? 'embedding' : 'chat',
        contextWindow: 4096,
        isFree: false,
        supportsStreaming: true,
        supportsFunctionCalling: false,
        supportsVision: false,
        description: `${base.provider} model`,
        tags: [base.provider],
      });
    }
  }

  // Generate additional variant models
  const sizes = ['7b', '13b', '34b', '70b'];
  const variants = ['chat', 'instruct', 'dpo', 'sft', 'rlhf'];
  const communityProviders = ['upstage', 'tiiuae', 'bigscience', 'eleutherai', 'mosaicml', 'stabilityai', 'rwkv', 'cerebras'];
  
  for (const prov of communityProviders) {
    for (const size of sizes.slice(0, 2)) {
      for (const variant of variants.slice(0, 2)) {
        extras.push({
          provider: prov,
          modelId: `${prov}/${prov}-${size}-${variant}`,
          displayName: `${prov.charAt(0).toUpperCase() + prov.slice(1)} ${size.toUpperCase()} ${variant}`,
          category: 'chat',
          contextWindow: 4096,
          isFree: false,
          supportsStreaming: true,
          supportsFunctionCalling: false,
          supportsVision: false,
          description: `Community model from ${prov}`,
          tags: [prov, 'open-source', 'community'],
        });
      }
    }
  }
  ...
  const specializedModels = [
    { provider: 'stability', id: 'stability/stable-diffusion-xl', name: 'SDXL', cat: 'image' },
    { provider: 'stability', id: 'stability/stable-diffusion-3', name: 'SD 3', cat: 'image' },
    { provider: 'stability', id: 'stability/stable-code-3b', name: 'Stable Code 3B', cat: 'code' },
    { provider: 'bigcode', id: 'bigcode/starcoder2-15b', name: 'StarCoder2 15B', cat: 'code' },
    { provider: 'bigcode', id: 'bigcode/starcoder2-7b', name: 'StarCoder2 7B', cat: 'code' },
    { provider: 'bigcode', id: 'bigcode/starcoder2-3b', name: 'StarCoder2 3B', cat: 'code' },
    { provider: 'codellama', id: 'codellama/codellama-13b-instruct', name: 'CodeLlama 13B', cat: 'code' },
    { provider: 'codellama', id: 'codellama/codellama-7b-instruct', name: 'CodeLlama 7B', cat: 'code' },
    { provider: 'wizardlm', id: 'wizardlm/wizardcoder-33b', name: 'WizardCoder 33B', cat: 'code' },
    { provider: 'wizardlm', id: 'wizardlm/wizardcoder-15b', name: 'WizardCoder 15B', cat: 'code' },
    { provider: 'thenlper', id: 'thenlper/gte-large', name: 'GTE Large', cat: 'embedding' },
    { provider: 'baai', id: 'baai/bge-large-en-v1.5', name: 'BGE Large', cat: 'embedding' },
    { provider: 'baai', id: 'baai/bge-small-en-v1.5', name: 'BGE Small', cat: 'embedding' },
    { provider: 'voyage', id: 'voyage/voyage-2', name: 'Voyage 2', cat: 'embedding' },
    { provider: 'voyage', id: 'voyage/voyage-code-2', name: 'Voyage Code 2', cat: 'embedding' },
  ];

  for (const m of specializedModels) {
    extras.push({
      provider: m.provider,
      modelId: m.id,
      displayName: m.name,
      category: m.cat,
      contextWindow: m.cat === 'embedding' ? 8192 : m.cat === 'image' ? 0 : 16384,
      isFree: false,
      supportsStreaming: m.cat !== 'image' && m.cat !== 'embedding',
      supportsFunctionCalling: false,
      supportsVision: false,
      description: `${m.cat === 'code' ? 'Code' : m.cat === 'embedding' ? 'Embedding' : m.cat === 'image' ? 'Image generation' : 'Chat'} model`,
      tags: [m.provider, m.cat],
    });
  }
